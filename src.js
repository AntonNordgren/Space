...
Game.start = function () {
    ...
    Game.spawnEnemies(Game.enemyCounter);
    Game.mainloop();
}
    ...
    if (Player.bullets.length !== 0) {
        for (let i = 0; i < Player.bullets.length; i++) {
            ...
            for (let j = 0; j < Game.enemies.length; j++) {
                if(checkCollision(Player.bullets[i], Game.enemies[j])) {
...
Enemy.update = function () {
    if (Game.enemies.length !== 0) {
        for (let i = 0; i < Game.enemies.length; i++) {
            ...
            else if (Game.enemies[i].pos.y > Game.canvas.height - Game.enemies[i].height || Game.enemies[i].pos.y < 0) {
